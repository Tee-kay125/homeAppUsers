...
class CCS_Base:
        ...
        if count > 1:
            if self._typeFormat[0:2] == 'ST' or self._typeFormat[0:2] == 'CH':
                ...
                if (isinstance(self._default, (int, float)) is True):
                    self._Value = [self._default] * count
                else:
                    self._Value = [None] * count
        else:
            self._Value = self._default
    ...
    def bValidate(self):
        ...
        # If the _Value of this CCS_Base is a list then don't do validation checking
        # The reason is because a CCS_Base of count > 1 might have a _Min and _Max then
