...
echo "sudo docker run --volume $full_path_to_SICD:/app/SICD --volume $full_path_to_pipes_config:/app/pipes_config --volume $full_path_to_scriptingFiles:/app/scriptingFiles -p $webmms_port_number:80 webmms" > run_webmms_ubuntu_18.04.sh

# Load webmms image to local docker
sudo docker load < webmms.tar.gz

# make sh script executable

sudo chmod +x ./run_webmms_ubuntu_18.04.sh
sudo chmod +x ./stop_docker_image.sh

echo "installation complete, copy your xml files to generated SICD directory and run './run_webmms_ubuntu_18.04.sh' to start WebMM"